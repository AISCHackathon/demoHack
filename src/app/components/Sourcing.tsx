...
type PartnerType = "all" | "manufacturer" | "fabric_supplier" | "trim_supplier" | "dye_house";
type Region = "all" | "asia" | "europe" | "americas";
type Certification = "GOTS" | "OEKO-TEX" | "BSCI" | "ISO 9001" | "GRS" | "Fair Trade";
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`p-1.5 rounded-[6px] transition-colors cursor-pointer ${
                viewMode === "grid" ? "bg-burgundy-950/[0.06] text-foreground" : "text-muted-foreground/40 hover:text-muted-foreground"
              }`}
              title="Grid view"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-[6px] transition-colors cursor-pointer ${
                viewMode === "list" ? "bg-burgundy-950/[0.06] text-foreground" : "text-muted-foreground/40 hover:text-muted-foreground"
              }`}
              title="List view"
            >
        ...
        <div className="flex items-center gap-1.5 mb-4 overflow-x-auto pb-0.5 -mx-1 px-1">
          {typeOptions.map((t) => (
            <button
              ...
              className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-[6px] rounded-full border transition-colors cursor-pointer ${
                typeFilter === t
              ...
              {typeCounts[t] > 0 && (
                <span
                  className={`min-w-[16px] h-[16px] rounded-full flex items-center justify-center ${
                    typeFilter === t ? "bg-cream/20 text-cream" : "bg-burgundy-950/[0.05] text-muted-foreground"
                  }`}
          ...
          <div className="flex items-center gap-1">
            {regionOptions.map((r) => (
              <button
                ...
                className={`flex items-center gap-1 px-2.5 py-[6px] rounded-[7px] border transition-colors cursor-pointer ${
                  regionFilter === r
                  ...
                  >
                    <Heart
                      className={`w-3 h-3 transition-colors ${
                        favorites.has(partner.id) ? "text-burgundy fill-burgundy/30" : "text-burgundy-950/20"
                      }`}
                    />
                  </button>
                  {/* Badges */}
          ...
          <div className="relative w-full max-w-[480px] bg-white flex flex-col overflow-hidden">
            ...
            <div className="flex-shrink-0 px-6 py-4 border-t border-burgundy-950/[0.04] flex items-center gap-2.5">
              <button
                ...
                className={`w-[40px] h-[40px] rounded-[8px] border flex items-center justify-center transition-colors cursor-pointer flex-shrink-0 ${
                  favorites.has(selectedPartner.id)
              ...
              >
                <Heart
                  className={`w-4 h-4 ${
                    favorites.has(selectedPartner.id) ? "text-burgundy fill-burgundy/20" : "text-muted-foreground/50"
                  }`}
                />
              </button>
              <a
